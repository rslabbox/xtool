...
impl DirFileEntryData {
    ...
    pub(super) fn created(&self) -> DateTime {
        DateTime::decode(self.create_date, self.create_time_1, self.create_time_0)
    }

    pub(super) fn accessed(&self) -> Date {
        Date::decode(self.access_date)
    }

    pub(super) fn modified(&self) -> DateTime {
...
impl DirEntryData {
    pub(super) fn serialize<E: IoError, W: Write<Error = Error<E>>>(
        &self,
        wrt: &mut W,
    ) -> Result<(), Error<E>> {
    ...
    pub(super) fn deserialize<E: IoError, R: Read<Error = Error<E>>>(
        rdr: &mut R,
    ) -> Result<Self, Error<E>> {
...
impl DirEntryEditor {
    ...
    pub(super) fn flush<IO: ReadWriteSeek, TP, OCC>(
        &mut self,
