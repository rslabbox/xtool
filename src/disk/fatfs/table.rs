...
impl FatTrait for Fat12 {
        ...
        Error<E>: From<S::Error>,
    {
        ...
        while cluster < end_cluster {
            let packed_val = match cluster & 1 {
                0 => fat.read_u16_le(),
...
impl FatTrait for Fat32 {
        ...
        Error<E>: From<S::Error>,
    {
        ...
        Ok(match val {
            0 if (0x0FFF_FFF7..=0x0FFF_FFFF).contains(&cluster) => {
                ...
                warn!(
                    "cluster number {} is a special value in FAT to indicate {}; it should never \
                     be seen as free",
                    cluster, tmp
                );
            ...
            n if (0x0FFF_FFF7..=0x0FFF_FFFF).contains(&cluster) => {
                ...
                warn!(
                    "cluster number {} is a special value in FAT to indicate {}; hiding potential \
                     FAT chain value {} and instead reporting as a bad sector",
                    cluster, tmp, n
                );
                FatValue::Bad // avoid accidental use or allocation into a FAT chain
            }
        ...
        Error<E>: From<S::Error>,
    {
        ...
        if value == FatValue::Free && (0x0FFF_FFF7..=0x0FFF_FFFF).contains(&cluster) {
            ...
            panic!(
                "cluster number {} is a special value in FAT to indicate {}; it should never be \
                 set as free",
                cluster, tmp
            );
        };
